...
# Configuration
VLLM_SERVER_URL = os.getenv("VLLM_SERVER_URL", "http://0.0.0.0:8000/v1")
...
# Async client
async_client = AsyncOpenAI(base_url=VLLM_SERVER_URL, api_key=os.getenv("OPENAI_API_KEY", "EMPTY"))
...
async def send_request_openai_async(model, messages, req_id, results, metrics, semaphore):
    async with semaphore:
        ...
        for _ in range(RETRIES):
            try:
                resp = await async_client.chat.completions.create(model=model, messages=messages, max_tokens=MAX_TOKENS)
                txt = resp.choices[0].message.content.strip()
                metrics.end_request(req_id, txt)
                print(f"[ASYNC] Request {req_id} completed")
                results.append({"index": req_id, "input": messages[-1]["content"], "output": txt})
                return
            except Exception as e:
                print(f"[ASYNC] Request {req_id} failed: {e}")
        results.append({"index": req_id, "input": messages[-1]["content"], "error": "All retries failed"})
...
def send_request_openai_sync(model, messages, req_id, results, metrics):
    ...
    for _ in range(RETRIES):
        try:
            resp = sync_client.chat.completions.create(model=model, messages=messages, max_tokens=MAX_TOKENS)
            txt = resp.choices[0].message.content.strip()
            metrics.end_request(req_id, txt)
            print(f"[SYNC] Request {req_id} completed")
            return {"index": req_id, "input": messages[-1]["content"], "output": txt}
        except Exception as e:
            print(f"[SYNC] Request {req_id} failed: {e}")
    return {"index": req_id, "input": messages[-1]["content"], "error": "All retries failed"}
